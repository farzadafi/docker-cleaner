...
@SpringBootTest
@ActiveProfiles("test")
public class DockerSmellLastUserIsRootIntegrationTest {
    ...
    @Test
    void shouldFixLastUserIsRootSmellInAllDockerfiles() {
        ...
        if (after > 0) {
            fail("Some Dockerfiles still contain LAST_USER_IS_ROOT smell after fixing. Check logs above.");
        }
    }
    ...
    private int countSmell(DockerAnalysisReport report) {
        ...
        for (SmellFinding finding : report.getFindings()) {
            if (finding.type() == SmellType.LAST_USER_IS_ROOT)
                count++;
        }
        return count;
    }
    ...
    private void printSummary(
        ...
        System.out.println("--------------------------------------");
        System.out.println("LAST_USER_IS_ROOT smells:");
