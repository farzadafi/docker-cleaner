...
@SpringBootTest
@ActiveProfiles("test")
public class DockerSmellAptGetNoInstallRecommendsIntegrationTest {
    ...
    @Test
    void shouldFixAptGetMissingNoInstallRecommendsInAllDockerfiles() {
        String folderPath = config.getDockerfilesFolderPath();
        List<File> dockerfiles = findDockerfiles(folderPath);
        if (dockerfiles.isEmpty()) fail("No Dockerfiles found in folder: " + folderPath);
        int before = 0;
        int after = 0;
        ...
        printSummary(dockerfiles.size(), before, after, stillSmellyFiles, crashedFiles);
        if (!crashedFiles.isEmpty())
            fail("Some files crashed during processing. Check logs above.");
        if (after > 0)
            fail("Some Dockerfiles still contain APT_GET_NO_INSTALL_RECOMMENDS_MISSING smell after fixing.");
    }

    private int countSmell(DockerAnalysisReport report) {
        int count = 0;
        for (SmellFinding finding : report.getFindings()) {
            if (finding.type() == SmellType.APT_GET_NO_INSTALL_RECOMMENDS_MISSING)
                count++;
        }
        return count;
    }
    ...
    private void scanFolder(File folder, List<File> result) {
        ...
        for (File f : files) {
            if (f.isDirectory()) {
                if (f.getName().equalsIgnoreCase("clean")) continue;
                scanFolder(f, result);
                continue;
            }
            if (isDockerfile(f)) {
    ...
    private boolean isDockerfile(File f) {
        ...
        return name.endsWith(".dockerfile") || name.contains("dockerfile");
    }

    private void printSummary(int totalFiles, int before, int after, List<String> stillSmellyFiles, List<String> crashedFiles) {
        System.out.println("======================================");
        System.out.println("Integration Batch Test Finished");
        System.out.println("Total files: " + totalFiles);
        System.out.println("--------------------------------------");
        System.out.println("APT_GET_NO_INSTALL_RECOMMENDS_MISSING smells:");
        ...
        if (!stillSmellyFiles.isEmpty()) {
            ...
            for (String f : stillSmellyFiles) System.out.println(" - " + f);
        }

        if (!crashedFiles.isEmpty()) {
            System.out.println("Files crashed during test:");
            for (String f : crashedFiles) System.out.println(" - " + f);
        }
    }
}