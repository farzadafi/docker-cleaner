...
@SpringBootTest
@ActiveProfiles("test")
public class DockerSmellAddInsteadOfCopyOrWgetIntegrationTest {
    ...
    @Test
    void shouldFixAddInsteadOfCopyOrWgetSmellInAllDockerfiles() {
        ...
        for (File file : dockerfiles) {
            try {
                ...
                // Analyze cleaned file WITHOUT fixing again:
                DockerFixResult cleanedAnalysis = service.analyzeAndFix(result.cleanedDockerfilePath());
                int afterCount = countSmell(cleanedAnalysis.reportBeforeFix());
        ...
        if (after > 0) {
            fail("Some Dockerfiles still contain ADD_INSTEAD_OF_COPY_OR_WGET smell after fixing. Check logs above.");
        }
    }
    ...
    private int countSmell(DockerAnalysisReport report) {
        ...
        for (SmellFinding finding : report.getFindings()) {
            if (finding.type() == SmellType.ADD_INSTEAD_OF_COPY_OR_WGET) {
                count++;
            }
        }
        return count;
